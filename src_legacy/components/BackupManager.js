...
const BackupManager = ({ data, saveData, onClose }) => {
  ...
  const downloadBackupFile = (backupData) => {
    ...
    element.download = `نسخة_احتياطية_كرزة_موبايل_${new Date().toISOString().split('T')[0]}.json`;
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.writeFile(workbook, `نسخة_احتياطية_كرزة_موبايل_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const exportToPDF = () => {
    try {
      ...
      // رأس الصفحة
      doc.setFontSize(16);
      doc.text('نسخة احتياطية من نظام كرزة موبايل 🍒', pageWidth / 2, yPosition, { align: 'center' });
      yPosition += 10;
      doc.setFontSize(10);
      ...
      doc.save(`نسخة_احتياطية_كرزة_موبايل_${new Date().toISOString().split('T')[0]}.pdf`);

...
// Helper للحصول على التوكن والـ store slug
const getAuthHeaders = () => {
  const token = localStorage.getItem('token');

  // Try to get slug from URL first (most reliable source)
  let storeSlug = null;
  if (typeof window !== 'undefined') {
    const pathParts = window.location.pathname.split('/');
    // URL format: /{storeSlug}/... or /{storeSlug}
    if (pathParts.length > 1 && pathParts[1] && !pathParts[1].startsWith('api') && pathParts[1] !== 'admin' && pathParts[1] !== 'register-store') {
      storeSlug = pathParts[1];
      // Always sync URL slug to localStorage for consistency
      if (storeSlug) {
  ...
  // Fallback to localStorage if URL doesn't have slug
  if (!storeSlug) {
    storeSlug = localStorage.getItem('currentStoreSlug');
  }

  // Final fallback: get from user object in localStorage
  if (!storeSlug) {
    try {
      const userStr = localStorage.getItem('user');
      if (userStr) {
        const user = JSON.parse(userStr);
        storeSlug = user?.storeSlug || user?.currentStore?.slug;
      }
    } catch (e) {
  ...
  // ALWAYS send x-store-slug if we have it (critical for multi-tenancy)
  if (storeSlug) {
...
export const useStorage = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const keys = Object.keys(API_ENDPOINTS).filter(key => {
        // Always allow super_admin or features included in the plan
        return isSuperAdmin || hasFeature(plan, key) || key === 'categories'; // categories is essential
      });
...
export const initializeStorage = () => {
  console.log('Storage initialized - MongoDB will be used');
};
...
export const clearLocalCache = () => {
  ...
  console.log('Local cache cleared successfully - session preserved');

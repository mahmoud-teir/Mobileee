...
export async function getAuthUser(request) {
  ...
  try {
    ...
    // Default to user's assigned store if no slug provided
    ...
    if (store && user.role !== 'super_admin' && user.storeId?.toString() !== store._id.toString()) {
      return null;
    }
    ...
    // Add storeSlug to toJSON output
